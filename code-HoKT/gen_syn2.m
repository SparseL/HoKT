function [simi,ave,nbCluster,fname,W_Cube]= gen_syn2(T,z,nbChange,state,blogSize,avgDegree);
% gen_syn2.m
...
for kk=2:1:T
    ...
    W(:,[cc(:,1); cc(:,2); cc(:,3); cc(:,4)]) = W;
    simi{kk} = zeros(1,kk-1);    %%simi{2}是2时刻的邻接矩阵和之前时刻的相似度

    W_Cube{kk} = W;
    GT_Cube{kk} = GT;
    for i = 1:(kk-1)
        simi{kk}(i)=similarity(W_Cube{1,kk},W_Cube{1,i});  %%相似度元胞数组
    end
    means(kk) = mean(simi{kk});
end
numMean=size(means,2);
ave=mean(means(2:numMean));
