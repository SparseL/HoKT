function HoKT(fileClasses,fileEdges)
%%根据相似度
...
%The execution of the code produces  a file Ris.txt containing the clusters
%obtained at each time stamp and a file .mat storing all the workspace
% 
% Clara Pizzuti July 2013
...
%                    For instance, if you want to execute the algorithm on the synthetic networks #2-birth and death
%                    the call is "run_DYNMOGA('../Greene/birthdeath/birthdeath','../Greene/birthdeath/birthdeath')
                    [blogSize nbCluster fname W_Cube simi ave]= gen_syn_fileInput(T,fileClasses,fileEdges)
                 else
                
                nbCluster = 4;
                
                blogSize = 128; %number of nodes
                fprintf('\niteration: %d',k);
...
% synthetic network generation
                   [simi,ave,nbCluster fname W_Cube] = gen_syn2(T,z,nbChange,state,blogSize,avgDegree);
...
%                     '.mat'];
                ...
                gen=100; %set the number of generations
                ...
                Z1 = run_HoKTmain(W_Cube,nbCluster,gen,popSize,CrossoverFraction,mutationRate,simi,ave);
                allZ1{k} = Z1;
